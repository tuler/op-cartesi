...
2. Batcher/proposer integration; snapshot-based reorg handling. What is left here is the L1 contract suite (`op-deployer`) so `op-proposer` has somewhere to propose, and persistence: blocks, outputs and machine snapshots are all still in memory.
