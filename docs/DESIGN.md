...
2. Batcher/proposer integration; snapshot-based reorg handling. The batcher is done. What is left is the L1 contract suite via `op-deployer`, `op-proposer` on top of it, and persistence — blocks, outputs, receipts and machine snapshots are all still in memory. Two frictions are already known, both from trying it: a devnet L1 is not a chain id `op-deployer` knows, so it needs `--intent-type custom` plus `op-deployer bootstrap implementations` to place OPCM on that L1 first; and its `inspect genesis` / `inspect rollup` outputs have to be discarded in favour of ours, since our L2 genesis comes from the machine, not from an op-geth allocs file. `inspect l1` gives the addresses, which is all we want from it. The one place its L2 assumption reaches L1 state is the `AnchorStateRegistry` starting anchor root, which must be our genesis output root.

   Note what this step does *not* unlock. Deploying `OptimismPortal` does not make withdrawals work: `proveWithdrawalTransaction` verifies an MPT storage proof of the L2ToL1MessagePasser account against the L2 state root, and ours is a Cartesi hash tree. Proposals landing on L1 is the deliverable here; a working bridge is step 3.
