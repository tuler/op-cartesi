...
only prose. Three of its sets exist today in [`conformance/`](../conformance):

| Vector set | Pins | Section |
|---|---|---|
| [`engine/sequencing.json`](../conformance/engine/sequencing.json) | a whole sequencing run as the JSON-RPC that crossed the wire | §4, §5.1 |
...
The transcript is the one that makes this document testable. It records the
engine `op-cartesi run` serves with no flags, so an implementation replays it
against its own binary rather than against a harness — three blocks built,
fetched, imported and adopted, plus a forkchoice on an unknown head. The
[`integration`](../integration) suite both records and replays it, and takes
an endpoint (`OP_CARTESI_TEST_ENGINE_URL`), so the same op-node type checks
run against any engine that listens.

The rest are still to be built, in this order of usefulness:

| Vector set | Shape |
|---|---|
| `engine` rejections | one recorded exchange per §4.3 rejection mode |
