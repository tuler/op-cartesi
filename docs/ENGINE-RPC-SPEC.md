...
Like [BLOCKS-SPEC §17](BLOCKS-SPEC.md), this document wants fixtures rather
than prose. The wire surface is the easier half to pin, because it is
transcript-shaped:
...
| `eth-call` | tagged report sets, and the return value or revert error each produces |
...
The `engine` set is the one that matters most: driven over authenticated
HTTP it is implementation-agnostic by construction, which is what turns the
existing [`integration`](../integration) suite — today an in-process harness
using `op-node`'s own wire types — into a cross-implementation test. Making
that harness take an endpoint is a prerequisite for a second engine, and can
be done against the Go one first.
