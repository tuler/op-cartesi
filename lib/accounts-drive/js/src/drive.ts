...
const err = {
    ...
    badWidth: (msg?: string) =>
        new AccountsDriveError("badWidth", msg ?? "balance width must be 8, 16 or 32"),
};
...
function validateConfig(c: NormalizedConfig): void {
    ...
    if (c.tableOffset < HEADER_SIZE || c.tableOffset % ss !== 0) {
        throw new Error(
            `table offset ${c.tableOffset}: want >= ${HEADER_SIZE} and a multiple of the slot size`,
        );
    }
    interface Region {
        name: string;
    ...
    if (c.registryOffset !== 0) {
        ...
        if (c.registryOffset < HEADER_SIZE) {
            // in-header placement (spec §5)
            if (c.registryOffset < MIN_REGISTRY_OFFSET || end > HEADER_SIZE) {
                throw new Error(
                    `in-header registry [${c.registryOffset},${end}) outside [${MIN_REGISTRY_OFFSET},${HEADER_SIZE})`,
                );
            }
        } else {
    ...
    if (c.profile === ProfileSparse) {
        ...
        if (c.sparseOffset < HEADER_SIZE || c.sparseOffset % sss !== 0) {
            throw new Error(
                `sparse offset ${c.sparseOffset}: want >= ${HEADER_SIZE} and a multiple of the slot size`,
            );
        }
    ...
    for (const r of regions) {
        if (r.off + r.len > c.driveLength) {
            throw new Error(
                `${r.name} [${r.off},${r.off + r.len}) outside drive of ${c.driveLength} bytes`,
            );
        }
    }
...
export class Drive {
    ...
    async reloadRegistry(): Promise<void> {
        ...
        if (n > this.cfg.registryCapacity) {
            throw new Error(
                `token count ${n} exceeds registry capacity ${this.cfg.registryCapacity}`,
            );
        }
        let col = 64;
        for (let i = 0; i < n; i++) {
            ...
            if (width !== 8 && width !== 16 && width !== 32) {
                throw err.badWidth(
                    `registry entry ${i}: balance width must be 8, 16 or 32 (${width})`,
                );
            }
    ...
    tokenByAddress(addr: AddressLike): ResolvedToken | null {
        ...
        for (let i = 0; i < this.tokens_.length; i++) {
            if (bytesEqual(this.tokens_[i].address, address)) {
                return {
                    id: i,
                    address: this.tokens_[i].address.slice(),
                    width: this.tokens_[i].width,
                };
            }
        }
        return null;
    ...
    async registerToken(addr: AddressLike, width: number): Promise<number> {
        ...
        if (this.cfg.profile === ProfileWide) {
            ...
            if (sum + width > this.cfg.slotSize) {
                throw err.tableFull(
                    `columns would need ${sum + width} bytes in a ${this.cfg.slotSize}-byte slot`,
                );
            }
        }
    ...
    async getTokenBalance(addr: AddressLike, id: number): Promise<bigint> {
        const address = toAddress(addr);
        if (isZero(address)) throw err.zeroAddress();
        if (!(Number.isInteger(id) && id >= 0 && id < this.tokens_.length))
            throw err.unknownToken();
...
export async function open(store: Store): Promise<Drive> {
    ...
    if (flags !== 0) {
        throw new Error(
            `unsupported accounts drive version: nonzero flags 0x${flags.toString(16)}`,
        );
    }
