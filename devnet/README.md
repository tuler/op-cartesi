...
- read its configuration from the environment the service sets —
  ...
  `HTTP_ADDR`, `JWT_SECRET_FILE`, and `CHAIN_CONFIG_FILE`, whose JSON document
  the `genesis` step writes with the consensus parameters every node of the
  chain must agree on ([BLOCKS-SPEC §4](../docs/BLOCKS-SPEC.md));
...
- **`devnet/chain-config.json`**, written by `compose/genesis.ts` and read by
  `compose/engine.sh`. It carries the chain's consensus parameters
  ([BLOCKS-SPEC §4](../docs/BLOCKS-SPEC.md)), which determine the L2 genesis
  block hash — so an engine serving a different chain than `rollup.json`
  describes is one op-node rejects. The parameters *below* genesis (the input
  envelope, the cycle budget) op-node cannot check at all, which is the better
  reason for one file: a disagreement there surfaces as a state root
  divergence at the first block that notices. Generated by the step that
  committed to them, read by the step that has to match — and being a document
  rather than a command line, readable by an engine that is not this one.
...
machine server you point `MACHINE_REMOTE` at. The parameters it uses must
match the ones the engine serves, which is why both come from one chain
configuration document (`op-cartesi config`) rather than from two lists — and
why, in the devnet, the genesis step writes that document and the engine is
started from it.
