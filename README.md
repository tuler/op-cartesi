...
The withdrawal trie and the cross-domain encodings are pinned in three directions: against the guest's TypeScript encoders, against geth's own trie verifier, and against the vendored Solidity — Optimism's real `SecureMerkleTrie`, `Encoding` and `Hashing`, so the bytes are judged by the exact code that will judge them on L1.

The first two of those now go through **[shared vector files](conformance/README.md)** rather than constants copied between suites: the node generates `conformance/`, and both the Go tests and the guest's own TypeScript replay it. Generation verifies before it writes — every trie proof through geth's verifier, every tree root against a naive builder, every header against its own hash — and asserts the constants the Solidity suite still hardcodes, so the three cannot drift apart silently.

```sh
go test ./chain -run TestConformance          # replay the vectors
go test ./chain -run TestConformance -update  # regenerate, then read the diff
```
...
| `conformance` | The vectors both implementations read ([conformance/README.md](conformance/README.md)): the genesis and block headers, the input envelope, the withdrawal and event encodings, the outputs tree and the withdrawal trie. Generated by `go test ./chain -run TestConformance -update`, replayed by the Go suite and by the guest's TypeScript. |
