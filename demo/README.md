...
[`@op-cartesi/app`](../guest/ts) with the devnet genesis (`CHAIN_ID`, `OWNER` from
...
- [`@op-cartesi/app`](../guest/ts) — the runtime: router, admission, the outcome
  model, the journaled ledger, the built-in handlers, and the application
  API (`app.contract({ address, abi, transactions, views })`).
- [`@op-cartesi/evm`](../lib/evm-compat/js) — the wire-level vocabulary:
  ...
  from here.
- [`@op-cartesi/abis`](../lib/abi-drive/js) — the ABI drive
...
- [`@op-cartesi/accounts`](../lib/accounts-drive/js) — the accounts drive itself.
...
The runtime's tests live with the runtime (`app`, `lib/abi-drive/js`):
...
[`@op-cartesi/app`](../guest/ts), which is what actually drives the CMIO loop.
